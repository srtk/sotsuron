...
\begin{itemize}
\item \textbf{costs} 最終レイヤーで誤差を計算するときに使われる、損失関数(loss function)を集めたパッケージである。主にSGDによって利用される。Dropout、Lpペナルティ、クロスエントロピー関数、Negative Log Likelihood関数などがここに含まれている。また、DBNやAutoencoderのように、独自の損失関数が必要な場合も、ここで定義されている。
\item \textbf{datasets} MNISTやCIFAR10, SVHNなどのデータセットがクラス化されている。新しいデータセットに対してDeep Learningを使いたい場合は、ここに定義されているDatasetクラスやDenseDesignMatrixクラスのサブクラスを作成することになる。
\item \textbf{distributions} ガウス分布などの分布関数が集められている。
\item \textbf{gui} 主に重みフィルタを可視化するためのクラスが用意されている。
\item \textbf{linear} Theanoの線形変換クラスを作り直したもので、Convolutional Layerもここに含まれる。
\item \textbf{models} Autoencoder、RBMなどの学習モデルが定義されている。
...
%\item \textbf{training_algorithms} SGDとBatch Gradient Descentを始めとする、学習アルゴリズムが定義されている。SGDと併用するための、学習率変化(線形減少、指数関数的減少、焼き鈍し)、慣性率、Polyak averagingなども、ここで一緒に定義されている。
